...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col">
      {/* Header */}
      <header className="safe-top bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm">
        <div className="px-4 py-3 flex items-center justify-between max-w-md mx-auto w-full">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500">Kalkulator Bending</p>
            </div>
          </div>
          {items.length > 0 && (
            <button
      ...
      <main className="flex-1 px-4 py-4 max-w-md mx-auto w-full pb-24">
        {tab === 'calc' && <Calculator onAddToQuotation={handleAddItem} />}
        {tab === 'quotation' && (
      ...
      {/* Bottom Nav */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 safe-bottom z-10 no-print shadow-[0_-1px_3px_rgba(0,0,0,0.05)]">
...
function NavBtn({ active, onClick, icon: Icon, label, badge }) {
  return (
    ...
    >
      ...
      {badge > 0 && (
        <span className="absolute top-1 right-1/4 bg-blue-700 text-white text-[9px] font-bold rounded-full min-w-[16px] h-4 flex items-center justify-center px-1">
          {badge}
        </span>
      )}
    </button>
